...
struct SCC
{
    int num[SCCNODE],low[SCCNODE],col[SCCNODE],
    int cycle[SCCNODE],st[SCCNODE],tail,cnt,cc;
    vector<int>adj[SCCNODE];
    ...
    void shrink(int n)
    {
        ...
        {
            reps(j,0,SZ(adj[i])-1)
                adj[i][j]=cycle[adj[i][j]];
                ///Careful. This will create self-loop
        }
        reps(i,0,n)
        {
            if(cycle[i]==i)
...
4. Dont repsget to keep space reps !A variables
*/
struct SAT2
{
    SCC scc;
    ...
    void atMostOneClause (int a[],int n,int flag)
    {
        ...
        reps(i,0,n)
        {
            reps(j,i+1,n)
            {
                orClause(a[i]^1,a[j]^1);
            /// !a || !b both being true not allowed
            }
        }
    }
    ...
    bool possible(int n)
    {
        ...
        reps(i,0,n)
        {
            ...
            if (scc.cycle[a]==scc.cycle[b])
                return false;
        }
        ///Valid solution exists
        return true;
    }
    ///To determine if A can be true. It cannot be true,
