...
int main()
{
	...
	int n,m,tmp;
	cin>>n>>m;
	//n and m degree of two polynomials
	n++; m++;
	for(int i=n;i--;)
    {
        // enter n+1 coefficients as c0+c1*x+c2*x*x+....
		cin>>tmp;
		coef1.pb(tmp);
	}
	tmp=1;
	...
	for(int i=m;i--;)
	{
	    // enter m+1 coefficients as c0+c1*x+c2*x*x+....
		scanf("%d",&tmp);
		coef2.pb(tmp);
	}
	tmp=1;
    ...
    {
		cout<<round(real(coef[i]))/tmp<<"  ";
    // coefficients of polynomial
    // which is product of two polunomials
    }
}
