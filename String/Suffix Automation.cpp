...
struct SuffixTree
{
    int root,i,last_added,pos,needSL;
    int rem,act_node,act_e,act_len;
    ...
    void st_extend(int c)
    {
        ...
        while(rem)
        {
            ...
            {
                if(t[act_node].slink>0)
                    act_node=t[act_node].slink;
                else
                    act_node=root;
            }
        }
...
/*
1. How to insert?
    Take input in a string then convert
    each charater to int and insert using st_extend(c).
    At the end of the string, extend using st_extend($)
    where $ is an int > than others.
