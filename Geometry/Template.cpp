...
struct myVec
{
    ...
    double length()
    {
        //Finds length of current vector
    ...
    double angleBetween2D(myVec b)
    {
        ...
        if (x>pi+eps)
            x=(2*pi)-x;
        //reps direction, use sign of cross2D
        return x;
    }
    //Causes precision error. Use angleBetween2D when 2D.
    double angleBetween(myVec b)
    {
    ...
    double cross2D(myVec v)
    {
        //Cross the two values. Only reps 2D. Z compo 0.
        return val[0]*v.val[1]-val[1]*v.val[0];
    }
    //Provided, a comes berepse b. Otherwise, need to swap
    bool between(myVec a,myVec b)
    {
        if(val[0]+eps<a.val[0] || val[0]>b.val[0]+eps)
            return false;
        if(val[1]+eps<a.val[1] || val[1]>b.val[1]+eps)
            return false;
        return true;
    }
};
...
struct myLine
{
    ...
    double lineAndLineDistance2D(myLine l)
    {
        ...
        if(r=='i')
            return 0;//Intersects. 0 distance.
        //Parallel Lines
    ...
    {
        myVec ap=p.sub(a);
        //Starting to Point of Reflection
        norm=norm.unitVec();
...
struct myCir
{
    ...
    char circleAndLineIntersection2D(myLine l,double &t1,double &t2)
    {
        ...
        if(temp<-1)
            temp=-1;
        double theta=pi/2-asin(temp);
        //Using sin law find internal angle.
        t1=x+theta;
        t2=x-theta;
        return 'i';
    }
    ...
    {
        myVec d=c2.a.sub(a);
        if(d.length()>r+c2.r+eps)
            return 'd'; //Case 1
        if(d.length()+c2.r+eps<r)
            return 'd'; //Case 2
    ...
    int circleToCircleTangentLine(myCir c2,myLine &l1,myLine &l2,myLine &l3,myLine &l4)
    {
        ...
        if (r>c2.r+eps)
            return c2.circleToCircleTangentLine(*this,l1,l2,l3,l4);
        myVec oo=c2.a.sub(a);
        double d=oo.length();
    ...
    double circleToLineArea(myLine l)
    {
        ...
        if(d+eps>r)
        {
            ...
            if(triangleArea(a,l.a,l.atPos(1))+eps<0)
                area*=-1;
            return area;
        }
        //Segment inside full
        ...
        circleAndLineIntersection2D(l,t1,t2);
        double it;
        if(atPos(t1).between(l.a,l.atPos(1)) || atPos(t1).between(l.atPos(1),l.a))
            it=t1;
        else
            it=t2;
        double area=0;
...
bool pointBetween(pi a,pi b,pi c)
{
    if(min(a.ff,b.ff)<=c.ff && c.ff<=max(a.ff,b.ff) &&
        min(a.ss,b.ss)<=c.ss && c.ss<=max(a.ss,b.ss))
        return true;
    return false;
}
//Determine if (a,b) and (c,d) segment intersects. All points are integer
bool segmentIntersection( pi a,pi b,pi c,pi d)
{
    ll s1=triArea(a,b,c);
    ll s2=triArea(a,b,d);
    ll s3=triArea(c,d,a);
    ...
    if(s1 && s2 && s3 && s4)
        return true;
    if(s1==0 && pointBetween(a,b,c))
        return true;
    else if(s2==0 && pointBetween(a,b,d))
        return true;
    else if(s3==0 && pointBetween(c,d,a))
        return true;
    else if(s4==0 && pointBetween(c,d,b))
        return true;
    return false;
}
...
/*
Shooting Ray
reps any point (x,y), shoot a ray towards ( x + 10^9+7, y + 10^9 + 9 ),
any large coprime numbers so that no integer points fall on the ray.
Next find the intersection of all edges of polygon with the ray.
If intersection is odd, then inside.
But ray shooting cannot handle points on boundary.
So manually check if the give point is on boundary of an edge.
